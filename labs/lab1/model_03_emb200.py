...
model_text_emb_200.add(Input(shape=(WINDOW_SIZE,)))
model_text_emb_200.add(Embedding(input_dim=MAX_WORDS_COUNT, output_dim=200, input_length=WINDOW_SIZE))
...
model_text_emb_200.add(BatchNormalization())
model_text_emb_200.add(Dense(writers_count, activation='softmax'))

compile_train_eval_model('model_03_Embedding200', model_text_emb_200,
                         x_train, y_train,
                         x_test, y_test,
                         class_labels=writers_list,
                         title='Embedding + Dense / 200')
