...
from tensorflow.keras.layers import Input, Dense, BatchNormalization, Embedding, Flatten
...
model_text_emb_200_nr.add(Input(shape=(WINDOW_SIZE,)))
model_text_emb_200_nr.add(Embedding(input_dim=MAX_WORDS_COUNT, output_dim=200, input_length=WINDOW_SIZE))
...
model_text_emb_200_nr.add(BatchNormalization())
model_text_emb_200_nr.add(Dense(writers_count, activation='softmax'))

compile_train_eval_model('model_04_Embedding200_without_layers', model_text_emb_200_nr,
                         x_train, y_train,
                         x_test, y_test,
                         class_labels=writers_list,
                         epochs=50,
                         title='Embedding + Dense / 200 без регуляризации')
